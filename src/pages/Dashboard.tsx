...
import { QrCode, LogOut, History, TrendingUp, CheckCircle, XCircle, Calendar, Info } from "lucide-react";
...
const Dashboard = () => {
  ...
  useEffect(() => {
    const checkAdmin = async () => {
      ...
      // Verificar se é admin ou producer usando função centralizada
  ...
  const loadStats = async () => {
    try {
      // Verificar se é producer para filtrar estatísticas
      ...
      if (isProducer) {
        query = query.eq('events.producer_id', session?.user?.id);
      }
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="mt-8">
          <Card className="shadow-medium hover:shadow-strong transition-shadow cursor-pointer" onClick={() => navigate("/about")}>
            <CardHeader>
              <div className="flex items-center space-x-2">
                <Info className="h-6 w-6 text-primary" />
                <CardTitle>Quem Somos Nós</CardTitle>
              </div>
              <CardDescription>
